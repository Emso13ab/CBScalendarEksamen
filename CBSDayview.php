<!DOCTYPE html>

<head>
<link type="text/css" rel="stylesheet" href="CBSDayview.css"/>
	<title>Dayview</title>
</head>
<style>
	body{
		background-image:url(sandpaper.png);
	}
</style>

<body>
	<div class="wrapper">
		<form action="CBScalendar.php" method="get">
			<button id="menu" type="submit" value="Back to weekview" name="Back to weekview">Back to weekview</button>
		</form>	
	</div>

	<table class="table">
		<tr>
			<td class="td3">November</td>
			<td class="td4">10.</td>
		</tr>

		<tr class="weekDay">
			<td class="td3"></td>
			<td class="td4">Monday</td>
		</tr>

		<tr>
			<td class="td3">08:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">09:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">10:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">11:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">12:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">13:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">14:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">15:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">16:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">17:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">18:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">19:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">20:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">21:00</td>
			<td class="td1"></td>
		</tr>

		<tr>
			<td class="td3">22:00</td>
			<td class="td1"></td>
		</tr>
	</table>	
</body>